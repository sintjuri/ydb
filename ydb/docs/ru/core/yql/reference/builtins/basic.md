...
Аргумент `<строка>` должен быть строковым литералом, известным на этапе
валидации запроса, а не произвольным выражением. Переменные, колонки, параметры
запроса и результаты функций не поддерживаются. Для преобразования значения во
время выполнения запроса используйте `CAST(выражение AS <Простой тип>)`.
Например, вычисляемое значение `String` следует преобразовывать в `Uuid` через
`CAST(выражение AS Uuid)`:
...
`Uuid($uuidAsString)` некорректен, потому что аргумент является выражением, а не
строковым литералом.
