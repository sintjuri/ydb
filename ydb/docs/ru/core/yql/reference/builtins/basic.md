...
Аргумент `<строка>` обозначает строковый литерал, а не произвольное выражение. Переменные, колонки, параметры запроса и результаты функций не поддерживаются. Для преобразования значения во время выполнения запроса используйте `CAST(выражение AS <Простой тип>)`. Например, вычисляемое значение `String` следует преобразовывать в `Uuid` через `CAST(выражение AS Uuid)`:
...
`Uuid($uuidAsString)` некорректен, потому что аргумент является выражением, а не строковым литералом.
