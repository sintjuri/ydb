...
When adding a column with the `NOT NULL` constraint to a [row-oriented table](../../../../concepts/datamodel/table.md#row-oriented-tables), you must also specify `DEFAULT`. The default value is used to populate existing rows. Column-oriented tables do not support adding a `NOT NULL` column because they do not support `DEFAULT` values.
